...
class Depth_Wise(Module):
    ...
    def __init__(self,
        ...
        c2_in, c2_out = c2
        c3_in, c3_out = c3
        self.conv = Conv_block(c1_in,
                               out_c=c1_out,
                               kernel=(1, 1),
                               padding=(0, 0),
                               stride=(1, 1))
        self.conv_dw = Conv_block(c2_in,
                                  c2_out,
                                  groups=c2_in,
                                  kernel=kernel,
                                  padding=padding,
                                  stride=stride)
        self.project = Linear_block(c3_in,
                                    c3_out,
                                    kernel=(1, 1),
                                    padding=(0, 0),
                                    stride=(1, 1))
...
class MiniFASNet(Module):
    ...
    def __init__(self,
        ...
        self.conv1 = Conv_block(img_channel,
                                keep[0],
                                kernel=(3, 3),
                                stride=(2, 2),
                                padding=(1, 1))
        self.conv2_dw = Conv_block(keep[0],
                                   keep[1],
                                   kernel=(3, 3),
                                   stride=(1, 1),
                                   padding=(1, 1),
        ...
        c1 = [(keep[4], keep[5]), (keep[7], keep[8]), (keep[10], keep[11]),
              (keep[13], keep[14])]
        c2 = [(keep[5], keep[6]), (keep[8], keep[9]), (keep[11], keep[12]),
              (keep[14], keep[15])]
        c3 = [(keep[6], keep[7]), (keep[9], keep[10]), (keep[12], keep[13]),
              (keep[15], keep[16])]
        ...
        self.conv_5 = Residual(c1,
                               ...
                               stride=(1, 1),
                               padding=(1, 1))
        self.conv_6_sep = Conv_block(keep[46],
                                     keep[47],
                                     kernel=(1, 1),
                                     stride=(1, 1),
                                     padding=(0, 0))
        self.conv_6_dw = Linear_block(keep[47],
                                      keep[48],
                                      groups=keep[48],
                                      kernel=conv6_kernel,
                                      stride=(1, 1),
...
class MiniFASNetSE(MiniFASNet):
    ...
    def __init__(self,
                 ...
                 num_classes=4,
                 img_channel=3):
        super(MiniFASNetSE, self).__init__(keep=keep,
...
def MiniFASNetV1(embedding_size=128,
                 conv6_kernel=(7, 7),
                 drop_p=0.2,
                 num_classes=3,
                 img_channel=3):
    return MiniFASNet(keep_dict['1.8M'], embedding_size, conv6_kernel, drop_p,
                      num_classes, img_channel)


# (80x80) flops: 0.044, params: 0.43
def MiniFASNetV2(embedding_size=128,
                 conv6_kernel=(7, 7),
                 drop_p=0.2,
                 num_classes=3,
                 img_channel=3):
    return MiniFASNet(keep_dict['1.8M_'], embedding_size, conv6_kernel, drop_p,
                      num_classes, img_channel)


def MiniFASNetV1SE(embedding_size=128,
                   conv6_kernel=(7, 7),
                   drop_p=0.75,
                   num_classes=3,
                   img_channel=3):
...
# (80x80) flops: 0.044, params: 0.43
def MiniFASNetV2SE(embedding_size=128,
                   conv6_kernel=(7, 7),
                   drop_p=0.75,
                   num_classes=4,
                   img_channel=3):
...
class FaceAntiM1(object):

    def __init__(self):
        self.device = torch.device(TORCH_DEVICE)
        self.model = MiniFASNetV2(conv6_kernel=(5, 5))
        state_dict = torch.load(Path(__file__).parent / 'face_anti_m1.pth',
                                map_location='cpu')
    ...
    def predict(self, image):
        """
        :param image:   tensor -- [1, 3, 80. 80]  scale 2.7
        :return:
        """
        with torch.no_grad():
...
class FaceAntiM2(object):

    def __init__(self):
        self.device = torch.device(TORCH_DEVICE)
        self.model = MiniFASNetV1SE(conv6_kernel=(5, 5))
        state_dict = torch.load(Path(__file__).parent / 'face_anti_m2.pth',
                                map_location='cpu')
        self.model.load_state_dict(state_dict)
        self.model.to(self.device)
        self.model.eval()
    ...
    def predict(self, image):
        """
        :param image:   tensor -- [1, 3, 80, 80] scale 4
        :return:
        """
        with torch.no_grad():
            result = self.model(image)
...
if __name__ == "__main__":
    ...
    model_1 = FaceAntiM1()
    model_2 = FaceAntiM2()
    img_1 = torch.ones((1, 3, 80, 80))
    img_1 = img_1.to(device)
    img_2 = torch.ones((1, 3, 80, 80))
    img_2 = img_2.to(device)
    result_1 = model_1.predict(img_1)
    result_2 = model_2.predict(img_2)
    print(result_1)
    print(result_2)
    result = result_1 + result_2
    anti_score, anti = torch.max(result, 1)
    print(f'anti : {anti.item()}')
    print(f'anti_score : {anti_score.item()}')
